...
agent_communication:
  ...
  - agent: "main"
    message: "🔄 МОДЕРНИЗАЦИЯ РАСПИСАНИЯ ЗАВЕРШЕНА: Система переведена на новое расписание обновлений 2 раза в день (09:00 и 19:00 МСК). Убраны ВСЕ мок данные - теперь только 100% реальные матчи из API. Добавлены статусы матчей (запланирован/идёт/завершён). Время матчей отображается реальное (не подменяется). Если API не отвечает - система сохраняет предыдущие реальные матчи вместо генерации фейковых. Обновлён фронтенд для отображения статусов и реального времени. Система готова к тестированию новых требований!"
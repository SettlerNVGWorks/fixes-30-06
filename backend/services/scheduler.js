...
class Scheduler {
  ...
  setupSchedules() {
    // Обновление матчей в 12:00 дня (по серверному времени)
    cron.schedule('0 12 * * *', async () => {
      console.log('🕛 Запуск обновления матчей в 12:00 дня');
    ...
    // Обновление матчей в 00:00 (полночь) - новые матчи на следующий день
    cron.schedule('0 0 * * *', async () => {
      console.log('🕛 Запуск обновления матчей в 00:00 (полночь)');
      await this.updateDailyMatches();
    ...
    console.log('   📅 Обновление матчей: 12:00 МСК и 00:00 МСК каждый день');
    console.log('   🧹 Очистка старых матчей: в 00:00 МСК');
    console.log('   📊 Всегда по 2 матча на каждый вид спорта (8 матчей всего)');
  }
